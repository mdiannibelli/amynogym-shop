...
import { updateAdminProduct } from "@/actions/products/update-admin-product";
...
import { deleteImage } from "@/actions/products/delete-image";
...
interface Props {
  product: Partial<Product> & {ProductImages?: ProductImages[]};
...
export const ProductForm = ({ product, categories }: Props) => {
  const router = useRouter()
    ...
    const onSubmit = async(data: FormInputs) => {
        ...
        const {ok, product:updatedProduct} = await updateAdminProduct(formData);
        if(!ok) {alert('No se ha podido actualizar el producto'); return;}
        
        router.replace(`/admin/productos/${updatedProduct?.slug}`)
    }
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="grid px-5 mb-16 grid-cols-1 sm:px-0 sm:grid-cols-2 gap-3">
      ...
      <div className="w-full">
        ...
        <button className="bg-blue-600 text-white text-md py-2 px-3 text-center hover:bg-blue-700 duration-300 w-full">
          Guardar
        </button>
      </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            {
                product.ProductImages?.map((image) => (
                    <div key={image.id}>
                        ...
                        <button onClick={() => deleteImage(image.id, image.url)} 
                        type="button" className="bg-red-600 text-white text-md py-2 px-4 text-center hover:bg-red-700 duration-300 w-full rounded-b-xl">Eliminar</button>
                    </div>
                ))
            }
          </div>
